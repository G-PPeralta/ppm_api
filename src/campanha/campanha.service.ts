      ...
      from (
        select 
          ...
          campanha.id as id_campanha,
            ...
            (select TO_CHAR(min(dat_ini_real), 'DD/MM/YYYY') from tb_camp_atv_campanha where id_pai = pai.id and ind_atv_execucao = 1) as inicioPlanejado_fmt,
            (select TO_CHAR(max(dat_fim_real), 'DD/MM/YYYY') from tb_camp_atv_campanha where id_pai = pai.id and ind_atv_execucao = 1) as finalPlanejado_fmt,
            ...
            (select min(dat_ini_real) from tb_camp_atv_campanha where id_pai = pai.id and ind_atv_execucao = 1) as inicioPlanejado,
